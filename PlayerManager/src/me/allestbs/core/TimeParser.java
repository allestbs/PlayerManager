...
public class TimeParser {
	...
	public static long parseString(String string) {
		...
		for (String st : list) {
			...
			if (st.length() != 1 && String.valueOf(ch).matches("[M,w,d,h,m,s]")) {
				// Total milliseconds
				amount = Math.abs(Integer.parseInt(st.substring(0,
						st.length() - 1)));
				switch (ch) {
				case 's':
					total += (amount * 1000);
	...
	/**
	 * @param milliseconds
	 *            Milliseconds to convert to words
	 * @param abbreviate
	 *            For example, if true, 293000 -> "10m-53s", otherwise
	...
	public static String parseLong(long milliseconds, boolean abbreviate) {
		...
		for (String s : units) {
			if (!abbreviate) {
				...
				and = s.equals(units.get(units.size() - 1)) ? "" : s
						.equals(units.get(units.size() - 2)) ? " and " : ", ";
